...
// Get the LoanID from URL
$loanID = $_GET['LoanID'] ?? null;

if ($loanID) {
    // Prepare the SQL query to fetch the collateral information based on LoanID
    $query = "SELECT * FROM collateral_info WHERE LoanID = ?";
    $stmt = $conn->prepare($query);
    
    if ($stmt) {
        // Bind the parameter
        $stmt->bind_param("i", $loanID); // Use the same variable name here
        $stmt->execute();
        $result = $stmt->get_result();
        
        // Fetch the data
        if ($data = $result->fetch_assoc()) {
            // Data fetched successfully, you can use $data to access the row
            // Example: echo $data['square_meters'];
        } else {
            echo "No collateral found for this LoanID.";
        }
    ...
    echo "LoanID is missing from the URL.";
}
